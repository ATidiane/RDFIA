...
class ZCATransformation(object):
    ...
    def __call__(self, tensor):
        ...
        if tensor.size(1) * tensor.size(2) * \
                tensor.size(3) != self.transformation_matrix.size(0):
            raise ValueError(
        ...
        transformed_tensor = torch.mm(
            flat_tensor - self.transformation_mean,
...
def epoch(data, model, criterion, optimizer=None):
    ...
    for i, (input, target) in enumerate(data):
        # Applying ZCA Transformation
        input = ZCATransformation(input)
...
def main(params):
    ...
    pd_acc_loss_avg.to_csv(
        'results/norm_loss_acc_avg.csv',
        index=False)
    pd_train_loss.to_csv('results/norm_train_loss.csv', index=False)
